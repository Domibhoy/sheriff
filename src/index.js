...
function canModerate(i){return i.memberPermissions?.has(PermissionFlagsBits.ModerateMembers)||i.memberPermissions?.has(PermissionFlagsBits.ManageGuild)}
...
client.on('interactionCreate',async i=>{try{if(!i.isChatInputCommand())return;const cfg=guildConfig(i.guildId);if(['infraction','promotion','config'].includes(i.commandName)&&!canModerate(i))return i.reply({content:'You need moderation permissions to use this command.',ephemeral:true});
if(i.commandName==='infraction'){const u=i.options.getUser('user');cfg.infractions[u.id]??=[];const sub=i.options.getSubcommand();if(sub==='view')return i.reply({content:`**${u.tag}** has **${cfg.infractions[u.id].length}** infraction(s).`});if(sub==='remove'){cfg.infractions[u.id].pop();save();return i.reply({content:`Removed the latest infraction from ${u}.`})}const reason=i.options.getString('reason'),id=infractionId();cfg.infractions[u.id].push({reason,at:Date.now(),moderator:i.user.id,id});const count=cfg.infractions[u.id].length;const m=await i.guild.members.fetch(u.id);const rule=[...cfg.promotions].sort((a,b)=>b.count-a.count).find(r=>count>=r.count&&!m.roles.cache.has(r.roleId));if(rule){const role=i.guild.roles.cache.get(rule.roleId);if(role&&role.position<i.guild.members.me.roles.highest.position)await m.roles.add(role,`Automatic promotion at ${count} infractions`)}save();const embed=new EmbedBuilder().setColor(0xF2C94C).setTitle('Infraction').setDescription(`An infraction has been issued to <@${u.id}>.`).addFields({name:'Reason',value:reason},{name:'Issued By',value:`<@${i.user.id}>`}).setFooter({text:id});return i.reply({embeds:[embed]});}
...
if(i.commandName==='appeal'){const reason=i.options.getString('reason'),ch=cfg.channels.appeals?i.guild.channels.cache.get(cfg.channels.appeals):i.channel,row=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`appeal:approve:${i.user.id}`).setLabel('Approve').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId(`appeal:deny:${i.user.id}`).setLabel('Deny').setStyle(ButtonStyle.Danger));const embed=new EmbedBuilder().setTitle('Ban Appeal').setDescription(render(cfg.messages.appeal,{user:`<@${i.user.id}>`,reason})).addFields({name:'User ID',value:i.user.id},{name:'Status',value:'Pending'});await ch.send({embeds:[embed],components:[row]});return i.reply({content:'Your ban appeal has been submitted.',ephemeral:true});}
}catch(e){console.error(e);if(i.isRepliable()&&!i.replied)await i.reply({content:'Something went wrong.',ephemeral:true});}});
